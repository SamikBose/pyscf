...
class AccelScale(_Integrator):
    """Accel scale algorithm
    ...
    # def __init__(self, method, desired_T: float, friction_coef: float = 1.0, **kwargs):
    def __init__(self, method, desired_T: float, **kwargs):
        self.desired_T: float = desired_T
        # self.friction_coef = friction_coef
        self.accel = None
        super().__init__(method, **kwargs)
        # self.alpha = np.exp(-friction_coef * self.dt)
    ...
    #     return rng.normal(
    #         0,
    #         2.0 * self.friction_coef * data.nist.BOLTZMANN * self.desired_T,
    #         size=(self.mol.natm, 3),
    #     )
    ...
    def _compute_accel(self):
        ...
        print(f"Desired: {self.T}, Actual: {self.temperature()}")

        # Difference between desired and current temperature normalized
        delta_temp = self.T - self.temperature()
        print(f"Difference: {self.T - self.temperature()}")

        # delta_temp_norm = (self.T - self.temperature()) / self.T
        # print(f"Delta temp norm: {delta_temp_norm}")

        # sigmoid = 1 / 1+e^-x
        # sigmoid = 1.0 / (1 + np.exp(-delta_temp))
        # doubled_sigmoid = 2 / (1 + delta_temp_norm)
        doubled_sigmoid = 2.0 / (1.0 + np.exp(-delta_temp))
        print(f"Doubled sigmoid: {doubled_sigmoid}")

        a = -doubled_sigmoid * grad / self._masses.reshape(-1, 1)
        # a = -1 * grad / self._masses.reshape(-1, 1)
        return e_tot, a
